...
async function callAI(prompt, maxTokens) {
  ...
  if (geminiKey) {
    try {
      ...
      if (resp.ok) {
        ...
        console.error(`Gemini ${resp.status} error:`, errBody.slice(0, 300));
      } else if (resp.status === 429) {
        // Rate limited — wait 3s and try once more before falling back
        console.log('Gemini 429 rate limit, retrying in 3s');
        await sleep(3000);
        const retry = await fetch(`${GEMINI_URL}?key=${geminiKey}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            contents: [{ parts: [{ text: prompt }] }],
  ...
  if (!resp.ok) {
    const errBody = await resp.text();
    console.error(`Anthropic ${resp.status} error:`, errBody.slice(0, 300));
...
exports.handler = async function (event, context) {
  // ── DIAGNOSTIC MODE ── visit the function URL directly in a browser (GET)
  // to see exactly which AI provider is failing and why. Safe to leave in:
  // it reveals no key values, only status codes and error messages.
  if (event.httpMethod === 'GET') {
    const diag = { time: new Date().toISOString(), model: GEMINI_MODEL };
    const geminiKey = process.env.GEMINI_API_KEY || process.env.Gemini_API_KEY;
    const anthropicKey = process.env.ANTHROPIC_API_KEY;
    diag.geminiKeyPresent = !!geminiKey;
    diag.geminiKeyEnding = geminiKey ? '...' + geminiKey.slice(-4) : null;
    diag.anthropicKeyPresent = !!anthropicKey;
    ...
    if (anthropicKey) {
      try {
        const r = await fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-api-key': anthropicKey, 'anthropic-version': '2023-06-01' },
          body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 5, messages: [{ role: 'user', content: 'Reply with the single word OK' }] }),
        });
        diag.anthropic = { status: r.status, ok: r.ok, body: (await r.text()).slice(0, 600) };
      } catch (e) { diag.anthropic = { fetchError: e.message }; }
    }
